...
def get_Proxy_Ips(limit):
    ...
    try:
        if FREQEUENCY_TIME % FREQEUENCY_LIMIT == 0:
            if limit == 0:
                r = db.session.query(Proxy_Ips).order_by(Proxy_Ips.used_times.desc()).all()
            else:
                r = db.session.query(Proxy_Ips).order_by(Proxy_Ips.used_times.desc()).limit(limit).all()
            IP_PORT_LIST = []
            for rr in r:
...
def add_sub_Proxy_Ip(ip_port, type):
    try:
        ip_obj = Proxy_Ips.query.filter_by(ip_port=ip_port).first()
        if type == 'add':
            ip_obj.used_times += 1
        elif type == 'sub':
            ip_obj.used_times -= 1
        ip_obj.change_time = datetime.datetime.now()
        db.session.commit()
    except:
        pass

...
@upload.route('/do',methods=['GET','POST'])
def do():
    ...
    if request.method == 'POST':
        ...
        for i in file:
            ...
            newplan = shiftplan(duty_date=duty_date_a, cn_name=wk, shift=shift)
            db.session.add(newplan)
        db.session.commit()
        plans = shiftplan.query.all()
        flash('Upload successfully', "info")
        return render_template('upload/do.html',plans=plans)
...
def login():
    if request.method == 'GET':
        if "username" not in session:
            ...
            return render_template('upload/logout.html',username=(session['username']))

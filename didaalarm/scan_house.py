...
def getblocks(payloadData):
    # postUrl = 'http://www.cq315house.com/WebService/Service.aspx/getParamDatas2'
...
if __name__ == "__main__":
    ...
    engine = create_engine(cfg.MYSQL_URI,
                           pool_pre_ping=True,max_overflow=0,  # 超过连接池大小外最多创建的连接
                           pool_size = 400,  # 连接池大小
  ...
  and soldout = 'N'
    ...
    if letsgoB:
        ...
        for pn in projectname_set:
            ...
            bns = getblocks(payloadData)
            for bn in bns:
                if bn["buildingid"] in proj_buil_id_list:
                    sold_count = 0
                    left_count = 0
                    rjs = getRoomJson(bn["buildingid"])
                    for rj in rjs:
                        if rj["use"] == "成套住宅":
                            house_found = True
                            if rj["roomstatus"] in ["网签", "认购"]:
